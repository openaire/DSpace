...
public class OpenAIREMatomoEventListener extends AbstractUsageEventListener {
    ...
    private synchronized void buildMatomoRequest(Item item, Bitstream bitstream, HttpServletRequest httpRequest) {
        ...
        customVars.put("1", new String[] {
            "oaipmhID",
            "oai:" + dspaceURL + ":" + item.getHandle()});
        customVars.put("2", new String[] {
            "timestamp",
            String.valueOf(ZonedDateTime.now(ZoneOffset.UTC).toEpochSecond())});

        matomoRequest.put("cvar", new Gson().toJson(customVars));
        matomoRequest.put("cdt", String.valueOf(ZonedDateTime.now(ZoneOffset.UTC).toEpochSecond()));
